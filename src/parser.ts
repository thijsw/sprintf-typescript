...
function parsePlaceholder(
  ...
  if (firstCh >= '1' && firstCh <= '9') {
    ...
    if (end === -1) {
      throw makeSyntaxError(
        'unterminated named-argument parenthesis',
        format,
        i,
      )
    }
  ...
  if (padLead === '0') {
    ...
    if (custom === '' || custom === '$') {
      throw makeSyntaxError(
        'invalid custom pad character after quote',
        format,
        i,
      )
    }
  ...
  if (format.charAt(i) === '.') {
    ...
    if (i === pStart) {
      throw makeSyntaxError(
        'precision `.` must be followed by at least one digit',
        format,
        i,
      )
    }
...
function parsePath(
  ...
  if (first === null) {
    throw makePathSyntaxError(
      'expected identifier at start of named-argument path',
      source,
      format,
      offset + i,
    )
  ...
  while (i < source.length) {
    ...
    if (ch === '.') {
      ...
      throw makePathSyntaxError(
        `unexpected character "${ch}" in named-argument path`,
        source,
        format,
        offset + i,
      )
...
function makeSyntaxError(
  ...
  return new SyntaxError(
    `sprintf: ${msg} at position ${position} in format string "${format}"`,
  )
}
