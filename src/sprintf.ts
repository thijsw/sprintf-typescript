 ...
 */
type ArgsFor<F extends string> = string extends F ? readonly unknown[] : ArgsOf<ParseFormat<F>>;

/** Likewise for the return type: only narrow when `F` is a literal. */
type ResultFor<F extends string, A extends readonly unknown[]> = string extends F
  ? string
  : FormatResult<ParseFormat<F>, A>;
