  ...
  switch (specifier) {
    case 's': {
      ...
      return placeholder.precision !== undefined
        ? s.slice(0, placeholder.precision)
        : s
    }
    case 'd':
    case 'i':
    ...
    case 'f': {
      const n = requireFloat(arg, specifier)
      return placeholder.precision !== undefined
        ? n.toFixed(placeholder.precision)
        : String(n)
    }
    case 'g': {
      const n = requireFloat(arg, specifier)
    ...
    case 'v': {
      const primitive =
        arg !== null &&
        arg !== undefined &&
        typeof (arg as { valueOf?: () => unknown }).valueOf === 'function'
 ...
 */
function applyAlignmentAndPadding(
  body: string,
  placeholder: Placeholder,
): string {
  let text = body
...
/**
 * Coerce a signed integer to its unsigned representation for `%o`, `%u`,
 * `%x`, `%X`. Non-negative values are returned as-is to preserve precision
 * beyond 32 bits; negatives fall back to 32-bit two's complement, matching
 * sprintf-js.
 */
