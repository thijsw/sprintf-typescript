...
describe('sprintf — named arguments', () => {
  ...
  it('resolves a nested key path', () => {
    expect(sprintf('%(user.name)s', { user: { name: 'Nested' } })).toBe('Nested');
  });

  it('resolves an index segment inside a path', () => {
    expect(sprintf('%(users[0].name)s', { users: [{ name: 'First' }] })).toBe('First');
  });
